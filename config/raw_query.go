...
const (
	...
	GetEmployeeById     = `SELECT id, name, email, division, position, role, contact, created_at, updated_at, deleted_at FROM mst_employee WHERE id = $1`
	GetEmployeeByEmail  = `SELECT id, name, email, password, division, position, role, contact, created_at, updated_at, deleted_at FROM mst_employee WHERE email = $1`
	GetEmployees        = `SELECT id, name, email, division, position, role, contact, created_at, updated_at, deleted_at FROM mst_employee LIMIT $1 OFFSET $2`
	...
	EmployeeById        = `UPDATE mst_employee SET deleted_at = (CURRENT_TIMESTAMP) WHERE id = $1`
	UpdateEmployeeById  = `UPDATE mst_employee SET name = $1, email = $2, password = $3, division = $4, position = $5, role = $6, contact = $7, updated_at = (CURRENT_TIMESTAMP) WHERE id = $8 RETURNING id, name, email, division, position, role, contact, created_at, updated_at`
	PagingEmployee      = `SELECT COUNT (*) FROM mst_employee`
)
